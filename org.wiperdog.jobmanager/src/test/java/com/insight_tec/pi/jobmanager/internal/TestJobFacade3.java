...
public class TestJobFacade3 {
	...
	public void jobFacadeTest1() throws Exception {
		...
		// 
		// いずれかのジョブが実行中になるまで待つ
		// （即座に終了してしまうジョブはこの方法ではチェックすることが出来ないので注意、そのようなジョブは、JobFacade.getJobReusult()を使う。
		while (true) {
		...
		// jobname2が一度開始されたかどうかチェック
		rlist = jf.getJobResult(jobname2);
		runningcount = jf.getJobRunningCount(jobname2);
		if (rlist.size() > 0 || runningcount > 0) {
			System.out.println(jobname2 + " is once started");
		}
		System.out.println("end of check");
		
		jc = jf.getJobClass(jcname1);
		// jobname1が今現在、JobClassの実行制限に引っかかっているかどうかチェック
		bvetoed = jc.isJobVetoed(jf.jobKeyForName(jobname1));
		System.out.println("jobname1 is vetoed: " + bvetoed);
		
		// jobname2が今現在、JobClassの実行制限に引っかかっているかどうかチェック
		bvetoed = jc.isJobVetoed(jf.jobKeyForName(jobname2));
		System.out.println("jobname2 is vetoed: " + bvetoed);
